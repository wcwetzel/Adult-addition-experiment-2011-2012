...
model {
	...
	logk ~ dnorm(0, 0.0001)
	#k2 <- exp(logk2)
	#logk2 ~ dnorm(0, 0.0001)
	
	# Likelihood
	for(i in 1:nPlants){
		G[i] ~ dpois(lambda[i])
		lambda[i] <- mu[i] * rho[i]
		mu[i] <- exp(R + q * Q[i] + b * F[i]) * F[i]
		rho[i] ~ dgamma(k, k)
		
		g11[i] ~ dpois(Q[i])
		#lambda2[i] <- Q[i] * rho2[i]
		#rho2[i] ~ dgamma(k2, k2)
	}
}
", fill = TRUE)
...
inits = function() list(R = 2, b=0, q=0, logk=0, O = ifelse(d$f>0, 1, 0), Q = d$g11) #, logk2=0)

# parms
parms = c('R', 'b', 'q', 'k', 'logk') #, 'k2', 'logk2')
...
####### R2jags #########
nb2fgerr = jags(data, inits, parms, 'adult-addition-galls-dd.txt', n.chains=3, n.iter=ni, n.thin=thin)
