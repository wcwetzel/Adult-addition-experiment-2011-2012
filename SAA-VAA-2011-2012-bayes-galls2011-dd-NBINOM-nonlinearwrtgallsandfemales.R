...
model {
	...
	for(i in 1:nPlants){
		G[i] ~ dpois(lambda[i])
		lambda[i] <- mu[i] * rho[i]
		mu[i] <- ((a + R * g11[i]) / (c + g11[i] + b * O[i])) * O[i]
		O[i] ~ dbin(p, F[i])
		rho[i] ~ dgamma(k, k)
		#pgr[i] <- lambda[i] / F[i]
	}
...
inits = function() list(x=0, a=1.5, R=6.8, b=1, c=-0.86, logk=0, O = ifelse(d$f>0, 1, 0))

# parms
parms = c('p', 'x', 'R', 'b', 'a', 'c', 'k', 'logk')
...
####### R2jags #########
nb2fgrat = jags(data, inits, parms, 'adult-addition-galls-dd.txt', n.chains=3, n.iter=ni, n.thin=thin)
